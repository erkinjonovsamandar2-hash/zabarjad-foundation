...
let jahonImg: string | undefined;
let bolalarImg: string | undefined;
let ozbekImg: string | undefined;

try { jahonImg = new URL("@/assets/nav/jahon.png", import.meta.url).href; } catch { jahonImg = undefined; }
try { bolalarImg = new URL("@/assets/nav/bolalar.png", import.meta.url).href; } catch { bolalarImg = undefined; }
try { ozbekImg = new URL("@/assets/nav/ozbek.png", import.meta.url).href; } catch { ozbekImg = undefined; }
...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[100] bg-white/80 dark:bg-[#0a0806]/85 backdrop-blur-xl border-b border-neutral-200/50 dark:border-white/10 shadow-sm transition-colors duration-300">
        ...
        <ul className="hidden md:flex items-center gap-7">
          {navLinks.map((link) => (
            <li key={link.href}>
              ...
              {link.href === "/library" ? (
                // ── Mega Menu Trigger (Logic isolated here) ─────────────────
                <div
                ...
                // ── HIGHLIGHTED "KITOB SOVCHILARI" STYLING (PINK) ─────────────────────
                <Link
                  to={link.href}
          ...
          {/* Controls */}
          <li className="flex items-center gap-4 pl-4 border-l border-neutral-300 dark:border-neutral-700">
            <LanguageSwitcher />
            <button
              onClick={() => setTheme(theme === "dark" ? "light" : "dark")}
              className="text-foreground transition-colors hover:text-amber-500 focus:outline-none drop-shadow-[0_1px_2px_rgba(255,255,255,0.8)] dark:drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]"
      ...
      {/* ── DESKTOP MEGA MENU (Rendered at root to guarantee perfect centering) ── */}
      <AnimatePresence>
        {megaMenuOpen && (
          <div 
